...
def download_forward_nacional(fecha):
    ...
    url = "https://www.spensiones.cl/apps/carteras/genera_xsl.php?fecpro={0}&listado=27".format(
        fecha_x)
...
def download_inversiones(fecha):
    if fecha == None:
        return
    fecha_x = "".join(str(fecha)[0:7].split("-"))
    url = "https://www.spensiones.cl/apps/carteras/genera_xsl.php?fecpro={0}&listado=1".format(
        fecha_x)
    # custom token añomesdia
...
def download_activos(fecha):
    if fecha == None:
        return
    fecha_x = "".join(str(fecha)[0:7].split("-"))
    url = "https://www.spensiones.cl/apps/carteras/genera_xsl.php?fecpro={0}&listado=3".format(
        fecha_x)
    # custom token añomesdia
...
def download_extranjeros(fecha):
    if fecha == None:
        return
    fecha_x = "".join(str(fecha)[0:7].split("-"))
    url = "https://www.spensiones.cl/apps/carteras/genera_xsl.php?fecpro={0}&listado=24".format(
        fecha_x)
    # custom token añomesdia
    ...
    for f in fondos:
        ...
        try:
            urllib.request.urlretrieve(
                url+f, token.format(f, fecha_x), data=req)
            out = "Valores de cuota y patrimonio AFP FONDO: {0} {1} descargados correctamente".format(
                f, fecha_x)
        except:
            out = "Warning: Valores de cuota y patrimonio AFP FONDO: {0} {0} no se pudo descargar".format(
                f, fecha_x)
        print(out)
...
def fetch_last_update(fecha):
    ...
    for f in fondos:
        ...
        try:
            ...
            dia, mes, año = confirmado.split('-')
            confirmado = "-".join([dia, meses[mes], año])

            disponible = dlistas[1][2]
            dia, mes, año = disponible.split('-')
            disponible = "-".join([dia, meses[mes], año])

            return confirmado, disponible

        except Exception as e:
            out = "Warning: Valores de cuota y patrimonio AFP FONDO: {0} {0} no se pudo leer".format(
                f, fecha_x)
            return out, str(e)
...
def forward_nacional(start_date, end_date):
    cols = ['Fecha', 'Nombre', 'Fondo_A', 'Fondo_B',
    ...
    for fecha in rrule(MONTHLY, dtstart=start_date, until=end_date):
        try:
            ...
            continue
        df_i.columns = cols[1:]
        index = df_i[df_i.Nombre ==
                     'MONEDA OBJETO: Dólar estadounidense(US$)'].index[0]
...
def dif_forward_nacional(dfc, dfv, df):
    ...
    df = pd.concat([dfc, dfv]).groupby(['Fecha'], as_index=False).agg(
        {'Fondo_A': 'sum', 'Fondo_B': 'sum', 'Fondo_C': 'sum', 'Fondo_D': 'sum', 'Fondo_E': 'sum', 'TOTAL': 'sum'})
...
def inversiones(start_date, end_date):
    cols = ['Nombre', 'MMUSD_A', 'Porcentaje_A', 'MMUSD_B', 'Porcentaje_B', 'MMUSD_C', 'Porcentaje_C',
            'MMUSD_D', 'Porcentaje_D', 'MMUSD_E', 'Porcentaje_E', 'MMUSD_TOTAL', 'Porcentaje_TOTAL']
    cols_f = ['Fecha']+cols
    ...
    for fecha in rrule(MONTHLY, dtstart=start_date, until=end_date):
        ...
        nac = df_nacional[df_nacional['Nombre'].str.match(
        ...
        intrumentos_bc = df_nacional[df_nacional['Nombre'].str.match(
            'Instrumentos Banco Central')]
        instrumentos_t = df_nacional[df_nacional['Nombre'].str.match(
        ...
        bonos_bancarios = df_nacional[df_nacional['Nombre'].str.match(
            'Bonos Bancarios')]
        bonos_bancarios.loc[:, ('Nombre')] = 'Bonos Bancarios'
        deposito_plazo = df_nacional[df_nacional['Nombre'].str.match(
            'Depósitos a Plazo')]
        deposito_plazo.loc[:, ('Nombre')] = 'Depósitos a Plazo'

        inter = df_internacional[df_internacional['Nombre'].str.match(
            'INVERSIÓN EXTRANJERA TOTAL')]
        inter.loc[:, ('Nombre')] = 'INVERSIÓN EXTRANJERA'
        inter_rv = df_internacional[df_internacional['Nombre'].str.match(
            'RENTA VARIABLE')]
        inter_rv.loc[:, ('Nombre')] = 'RENTA VARIABLE'
        inter_rf = df_internacional[df_internacional['Nombre'].str.match(
            'RENTA FIJA')]
        inter_rf.loc[:, ('Nombre')] = 'RENTA FIJA'

        df_total_activos = df_total_activos.append(
            total_activos, ignore_index=True, sort=False)
...
def activos(start_date, end_date):
    cols = ['Nombre', 'Porcentaje_A', 'Porcentaje_B', 'Porcentaje_C',
            'Porcentaje_D', 'Porcentaje_E', 'Porcentaje_TOTAL', 'MM_TOTAL', 'MMUSD_TOTAL']
    cols_f = ['Fecha']+cols
...
def extranjeros(start_date, end_date):
    cols = ['Nombre', 'ASIA_EMERGENTE', 'ASIA_PACIFICO_DESARROLLADA', 'EUROPA', 'EUROPA_EMERGENTE',
            'LATINOAMERICA', 'MEDIO_ORIENTE_AFRICA', 'NORTEAMERICA', 'OTROS', 'MMUSD_TOTAL']
    cols_f = ['Fecha']+cols
    ...
    for fecha in rrule(MONTHLY, dtstart=start_date, until=end_date):
        ...
        df_total_ext = df_total_ext.append(
            total, ignore_index=True, sort=False)
        file.close()
...
def valor_fondos_historic(start_date, end_date):
    ...
    VF = pd.DataFrame(
        columns=['Fecha', 'VF_A', 'VF_B', 'VF_C', 'VF_D', 'VF_E'])
...
def usdclp_hist():
    ...
    usdclp_hist['Date'] = pd.to_datetime(
        usdclp_hist['Date'], format='%b %d, %Y')
...
def monthly_delete_by_date(fecha):
    # estos son mensuales
    input_rows = session.query(FORWARDS_NACIONALES).filter(
        FORWARDS_NACIONALES.Fecha == fecha).delete()

    input_rows = session.query(INVERSION_NACIONAL).filter(
        INVERSION_NACIONAL.Fecha == fecha).delete()

    input_rows = session.query(INVERSION_INTERNACIONAL).filter(
        INVERSION_INTERNACIONAL.Fecha == fecha).delete()

    input_rows = session.query(INVERSION_TOTAL).filter(
        INVERSION_TOTAL.Fecha == fecha).delete()

    input_rows = session.query(ACTIVOS).filter(ACTIVOS.Fecha == fecha).delete()

    input_rows = session.query(EXTRANJEROS).filter(
        EXTRANJEROS.Fecha == fecha).delete()
...
def daily_delete_by_date(fecha):
    # estos son diarios
    input_rows = session.query(VALOR_FONDOS).filter(
        VALOR_FONDOS.Fecha == fecha).delete()
...
def delete_usdclp_dates(start_date, end_date):
    ...
    input_rows = session.query(USDCLP).filter(
        USDCLP.Fecha >= start_date, USDCLP.Fecha <= end_date).delete()
    session.commit()
...
def upload_to_sql_monthly(start_date, end_date=None):
    ...
    df_total_activos, df_nacional, df_internacional = inversiones(
        start_date, end_date)
...
def query_by_daterange(label, start_date, end_date):
    ...
    if label == 'forwards_nacionales':
        input_rows = session.query(FORWARDS_NACIONALES).filter(
            FORWARDS_NACIONALES.Fecha.between(start_date, end_date))
    elif label == 'inversion_nacional':
        input_rows = session.query(INVERSION_NACIONAL).filter(
            INVERSION_NACIONAL.Fecha.between(start_date, end_date))
    elif label == 'inversion_internacional':
        input_rows = session.query(INVERSION_INTERNACIONAL).filter(
            INVERSION_INTERNACIONAL.Fecha.between(start_date, end_date))
    elif label == 'inversion_total':
        input_rows = session.query(INVERSION_TOTAL).filter(
            INVERSION_TOTAL.Fecha.between(start_date, end_date))
    elif label == 'activos':
        input_rows = session.query(ACTIVOS).filter(
            ACTIVOS.Fecha.between(start_date, end_date))
    elif label == 'extranjeros':
        input_rows = session.query(EXTRANJEROS).filter(
            EXTRANJEROS.Fecha.between(start_date, end_date))
    elif label == 'valor_fondos':
        input_rows = session.query(VALOR_FONDOS).filter(
            VALOR_FONDOS.Fecha.between(start_date, end_date))
    elif label == 'q_index':
        input_rows = session.query(Q_INDEX).filter(
            Q_INDEX.Fecha.between(start_date, end_date))
    elif label == 'usdclp':
        input_rows = session.query(USDCLP).filter(
